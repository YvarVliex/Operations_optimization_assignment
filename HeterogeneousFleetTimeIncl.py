...
q[0] = 0
p = q
dif = {key: q[key]-p[key] for key in p.keys()}
...
arc_pax = [(i,j,k) for i in nodes for j in nodes for k in vehicles if i!=j]
...
u = model.addVars(arc_pax, vtype=gb.GRB.INTEGER, name = 'u')
...
model.addConstrs((x[i,j,k] == 1) >> (u[i,j,k] + q[j] - p [j] == u[i,j,k]) for i in clients for j in nodes for k in vehicles if i!=j)
...
# model.addConstrs(gb.quicksum(p[i]*gb.quicksum(x[i,j,k] for j in nodes if i!= j) for i in clients) <= Q[k] for k in vehicles)
