...
q = {0:0, 1:25, 2:25, 3:25, 4:25, 5:25}
p = {0:0, 1:25, 2:25, 3:25, 4:25, 5:25}
#q[0] = 0
...
arc_pax = [(i,k) for i in nodes for k in vehicles]
...
u = model.addVars(arc_pax, vtype = gb.GRB.CONTINUOUS, name = 'u')   # dec variable local cap
...
model.addConstrs((x[i,j,k] == 1) >> (u[i,k] + q[j] - p[j] == u[j,k]) for i in clients for j in clients for k in vehicles if i!=j)
# model.addConstrs((x[i,j,k] == 1 ) >>  (t[i,k]+s[i] + times[i,j] == t[j,k]) for i in clients for j in clients for k in vehicles if i!=j)


# model.addConstrs(u[i,k] >= q[i] for i in clients for k in vehicles)
# model.addConstrs(u[i,k] <= p[i] for i in clients for k in vehicles)
